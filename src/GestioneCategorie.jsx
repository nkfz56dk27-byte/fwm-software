...
export default function GestioneCategorie({ onClose }) {
  ...
  async function caricaDati() {
    ...
    const { data: categorieDB } = await supabase
      .from('categorie_weekend')
      .select('*')
      .order('created_at', { ascending: true })
    
    const { data: redattoriDB } = await supabase
      .from('utenti')
      .select('username, nome_completo')
      .order('nome_completo')
    
    const { data: gruppiDB } = await supabase
      .from('gruppi_redattori')
      .select('*')
    
    setCategorie(categorieDB || [])
    setRedattori(redattoriDB || [])
    setGruppi(gruppiDB || [])
    setLoading(false)
  }

  async function eliminaCategoria(id) {
    if (!confirm('Eliminare questa categoria? I weekend associati non verranno eliminati.')) return
  ...
  async function toggleRedattore(categoriaId, username) {
    const esiste = gruppi.find(g => g.categoria_id === categoriaId && g.username === username)
    
    if (esiste) {
      await supabase.from('gruppi_redattori').delete().eq('id', esiste.id)
    } else {
      await supabase.from('gruppi_redattori').insert({ categoria_id: categoriaId, username })
    }
    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#f5f5f7' }}>
          ...
          {categorie.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px', color: '#999' }}>
              ...
              <div style={{ fontSize: '14px' }}>
                Clicca "+ Nuova Categoria" per iniziare
              </div>
            </div>
          ) : (
                ...
                return (
                  <div 
                    ...
                    style={{ 
                      ...
                      borderLeft: `6px solid ${categoria.colore}`
                    }}
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                      <div>
                        ...
                        <div style={{ fontSize: '13px', color: '#666' }}>
                          {redattoriCategoria.length} redattori assegnati
                        </div>
                      </div>
                    ...
                    <div style={{ fontSize: '15px', fontWeight: '600', marginBottom: '12px', color: '#333' }}>
                      Redattori assegnati:
                    </div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '10px' }}>
                      {redattori.map(redattore => {
                        ...
                        return (
                          <button
                            key={redattore.username}
                            onClick={() => toggleRedattore(categoria.id, redattore.username)}
                            style={{
                              padding: '12px 15px',
                              background: isAssegnato ? '#34C7591A' : '#f5f5f7',
                              border: isAssegnato ? '2px solid #34C759' : '2px solid transparent',
                              borderRadius: '10px',
                              cursor: 'pointer',
                              textAlign: 'left',
                              fontSize: '14px',
                              fontWeight: isAssegnato ? '600' : 'normal',
                              color: isAssegnato ? '#34C759' : '#666',
      ...
      {showNuovaCategoria && (
        <ModalCategoria 
          ...
          onSave={() => { setShowNuovaCategoria(false); caricaDati() }} 
        />
      )}
      ...
      {editCategoria && (
        <ModalCategoria 
          ...
          onSave={() => { setEditCategoria(null); caricaDati() }} 
        />
      )}
    </div>
  )
