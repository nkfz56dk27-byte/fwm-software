  ...
  async function caricaProssimoEvento() {
    try {
      ...
      const eventiFuturi = eventi.filter(evento => {
        ...
        // Se l'evento ha data_fine, controlla se termina oggi o dopo
        if (evento.data_fine) {
          const dataFine = new Date(evento.data_fine)
          dataFine.setHours(0, 0, 0, 0)
          // L'evento è attuale/futuro se: termina oggi O dopo
          return dataFine >= oggi
        }
        
        // Se non ha data_fine, controlla solo data_inizio
      ...
      // Se l'evento è già iniziato (data_inizio < oggi), usa oggi come data di riferimento
      // Altrimenti usa data_inizio
      const dataRiferimento = dataProssimo < oggi ? new Date(oggi) : new Date(dataProssimo)
      const giorniMancantiProssimo = Math.floor((dataRiferimento.getTime() - oggi.getTime()) / (1000 * 60 * 60 * 24))
      
      // Trova tutti gli eventi dello stesso giorno del prossimo evento (che può essere oggi se è già iniziato)
      const eventiStessoGiorno = eventiFuturi.filter(evento => {
        const dataEvento = new Date(evento.data_inizio)
        dataEvento.setHours(0, 0, 0, 0)
        // Se l'evento è già iniziato, consideralo come dello stesso giorno di oggi
        if (dataEvento < oggi) {
