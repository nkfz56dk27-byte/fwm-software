...
async function saveFirebaseToken(username, token, user_uid = null) {
  ...
  try {
    ...
    // Blocca se la sessione non è attiva
    if (!sessionResult || !sessionResult.data || !sessionResult.data.session) {
      console.error('[ERRORE] Nessuna sessione attiva! Impossibile salvare il token. Effettua il login prima di attivare le notifiche.', { sessionResult });
      return;
    } else {
