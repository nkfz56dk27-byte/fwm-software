...
export default function CalendarioAccrediti({ utenteCorrente, onClose }) {
  ...
  return (
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#f5f5f7' }}>
      ...
      {showNuovoEvento && <NuovoEventoModal campionati={campionati} onClose={() => setShowNuovoEvento(false)} onSave={caricaDati} utenteCorrente={utenteCorrente} />}
      ...
      {eventoSelezionato && <DettaglioEventoModal evento={eventoSelezionato} campionati={campionati} prenotazioni={prenotazioni} utenti={utenti} isAdmin={isAdmin} utenteCorrente={utenteCorrente} onClose={() => setEventoSelezionato(null)} onUpdate={caricaDati} />}
    </div>
  )
}
      ...
      <div style={{ background: 'white', borderRadius: '15px', width: '550px', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '20px 30px', borderTop: '1px solid #e0e0e0' }}>
          {isAdmin ? <div style={{ display: 'flex', gap: '10px' }}>
            <button onClick={elimina} style={{ flex: 1, padding: '12px', background: '#FF3B30', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', fontWeight: 'bold', fontSize: '18px' }}>✕ Elimina</button>
