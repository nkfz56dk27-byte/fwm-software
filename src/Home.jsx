...
function App() {
    ...
    // Mostra il tasto se le notifiche non sono attive e non è già stato richiesto
    useEffect(() => {
  ...
  useEffect(() => {
    if (user && user.username) {
      ...
      // 1. Non è mai stato chiesto prima, OPPURE
      // 2. È stato chiesto ma l'utente non ha ancora concesso il permesso e non ha cliccato definitivamente "Rifiuta"
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const { data, error } = await supabase.auth.signInWithPassword({
        email: username, // oppure email se usi email come login
        password: password
      });
      ...
      // Recupera i dati utente dalla tabella utenti
  ...
  const handleLogout = () => {
    ...
    // Rimuovi flag per forzare richiesta permessi notifiche
    localStorage.removeItem('notifichePrompted')
  }
        ...
        <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
          ...
          <div style={{ flex: 1 }}>
            <div className="home-cards-row">
              <div className="home-card card-blue" onClick={() => { console.log('🔵 CLICK CLASSIFICHE'); onOpenClassificheMainMenu() }} style={{ cursor: 'pointer' }}>
              ...
              <div className="home-card card-orange" onClick={onOpenCalendario} style={{ cursor: 'pointer', position: 'relative' }}>
...
function GestioneUtentiView({ onClose }) {
  ...
  const [loading, setLoading] = useState(true)
  const [actionLoading, setActionLoading] = useState({}) // { [utente.id]: true/false }
  ...
  const cambiaRuolo = async (utente) => {
    setActionLoading(al => ({ ...al, [utente.id]: true }))
    ...
    if (error) {
      alert('Errore Supabase (cambiaRuolo): ' + JSON.stringify(error));
      console.error('Supabase error cambiaRuolo:', error);
    } else {
      caricaUtenti();
    }
    setActionLoading(al => ({ ...al, [utente.id]: false }))
  }

  const resetPassword = async (utente) => {
    setActionLoading(al => ({ ...al, [utente.id]: true }))
    console.log('DEBUG resetPassword utente:', utente);
    if (!confirm(`Resettare la password di ${utente.username} a quella iniziale?`)) {
      setActionLoading(al => ({ ...al, [utente.id]: false }))
      return;
    }
    if (!utente.id) {
      alert('Errore: id utente mancante!');
      setActionLoading(al => ({ ...al, [utente.id]: false }))
      return;
    }
    const { error: errorReset } = await supabase.from('utenti').update({ password: 'FWM2025APP!?!', deve_cambiare_password: true }).eq('id', utente.id);
    if (errorReset) {
      alert('Errore Supabase (resetPassword): ' + JSON.stringify(errorReset));
      console.error('Supabase error resetPassword:', errorReset);
    } else {
      caricaUtenti();
    }
    setActionLoading(al => ({ ...al, [utente.id]: false }))
  ...
  const handlePasswordChangeAdmin = async (utente) => {
    setActionLoading(al => ({ ...al, [utente.id]: true }))
    console.log('DEBUG handlePasswordChangeAdmin utente:', utente);
    if (!utente.id) {
      alert('Utente senza id!');
      setActionLoading(al => ({ ...al, [utente.id]: false }))
      return;
    }
    const nuovaPassword = prompt('Nuova password per ' + utente.username);
    if (!nuovaPassword) {
      setActionLoading(al => ({ ...al, [utente.id]: false }))
      return;
    }
    const { error: errorChange } = await supabase.from('utenti').update({ password: nuovaPassword, deve_cambiare_password: false }).eq('id', utente.id);
    if (errorChange) {
      alert('Errore Supabase (cambio password): ' + JSON.stringify(errorChange));
      console.error('Supabase error cambio password:', errorChange);
    } else {
      caricaUtenti();
    }
    setActionLoading(al => ({ ...al, [utente.id]: false }))
            ...
            {utenti.map(utente => (
              <div key={utente.id} className="utente-row">
                ...
                <div className="utente-actions">
                    <button className="btn-action btn-green" onClick={() => setEditUtente(utente)} disabled={!!actionLoading[utente.id]}><svg className="icon-small" viewBox="0 0 24 24" fill="currentColor"><path d="M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z"/></svg>Modifica</button>
                    <button className="btn-action btn-blue" onClick={() => cambiaRuolo(utente)} disabled={!!actionLoading[utente.id]}><svg className="icon-small" viewBox="0 0 24 24" fill="currentColor"><path d="M12 4V1L8 5l4 4V6c3.31 0 6 2.69 6 6 0 1.01-.25 1.97-.7 2.8l1.46 1.46C19.54 15.03 20 13.57 20 12c0-4.42-3.58-8-8-8zm0 14c-3.31 0-6-2.69-6-6 0-1.01.25-1.97.7-2.8L5.24 7.74C4.46 8.97 4 10.43 4 12c0 4.42 3.58 8 8 8v3l4-4-4-4v3z"/></svg>Cambia Ruolo {actionLoading[utente.id] && '...'}</button>
                    <button className="btn-action btn-orange" onClick={() => resetPassword(utente)} disabled={!!actionLoading[utente.id]}><svg className="icon-small" viewBox="0 0 24 24" fill="currentColor"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM9 6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9V6z"/></svg>Reset Password {actionLoading[utente.id] && '...'}</button>
                    <button className="btn-action btn-yellow" onClick={() => handlePasswordChangeAdmin(utente)} disabled={!!actionLoading[utente.id]}><svg className="icon-small" viewBox="0 0 24 24" fill="currentColor"><path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z"/></svg>Cambia Password {actionLoading[utente.id] && '...'}</button>
                </div>
              </div>
            ))}
          </div>
